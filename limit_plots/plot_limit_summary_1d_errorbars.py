...
PB_TO_FB = 1000.0
BR_H_TO_BB = 0.582
BR_TTBAR_TO_2Q1L1NU = 0.439
BR_SCALE_TO_XYH_TT = 1.0 / (BR_H_TO_BB * BR_TTBAR_TO_2Q1L1NU)
LIMIT_TO_FB = PB_TO_FB * BR_SCALE_TO_XYH_TT

SCALAR_X_FB = 51.55
PSEUDOSCALAR_X_FB = 63.57
...
def draw_expected_errorbars(
    ...
    *,
    ...
    y_values = [point.expected * LIMIT_TO_FB for point in points]
    err_1_low = [(point.expected - point.expected_m1sigma) * LIMIT_TO_FB for point in points]
    err_1_high = [(point.expected_p1sigma - point.expected) * LIMIT_TO_FB for point in points]
    err_2_low = [(point.expected - point.expected_m2sigma) * LIMIT_TO_FB for point in points]
    err_2_high = [(point.expected_p2sigma - point.expected) * LIMIT_TO_FB for point in points]
...
def make_plot(points: list[LimitSummaryPoint], label: str, output_path: Path) -> None:
    ...
    ymax = max(point.expected_p2sigma * LIMIT_TO_FB for point in points) * 1.15
    ...
    ax.set_ylabel(
        r"$\sigma \times \mathcal{B}(X \to YH \to t\bar{t})$ [fb]",
        fontsize=12,
        labelpad=12,
    )
    ax.set_title(
        f"{label} discriminator\ncorrected for BR(H->bb) and BR(tt->SL)",
        fontsize=15,
        pad=18,
        fontweight="bold",
    )   
    ...
    fig.subplots_adjust(left=0.10, right=0.80, top=0.88, bottom=0.25)
...
def make_overlay_plot(
    ...
    ymax = max(
        point.expected_p2sigma * LIMIT_TO_FB
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only plot mass points with mX <= this value.",
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    for label, path in args.input:
        ...
        if args.max_mass_x is not None:
            points = [point for point in points if point.mass_x <= args.max_mass_x]

        if not points:
            raise SystemExit(f"No rows left after applying mass filters for {path}")
