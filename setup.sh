...
setup_xyh() {
    ...
    #   - "XYH_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "XYH_" variables is documented below.
    #
    ...
    cf_export_bool XYH_SETUP
    if ${XYH_SETUP} && ! ${CF_ON_SLURM}; then
        >&2 echo "the xyh analysis was already succesfully setup"
    ...
    #
    # global variables
    # (XYH = xyh, CF = columnflow)
    #

    # start exporting variables
    export XYH_BASE="${this_dir}"
    export CF_BASE="${cf_base}"
    export CF_REPO_BASE="${XYH_BASE}"
    export CF_REPO_BASE_ALIAS="XYH_BASE"
    ...
    if ! ${CF_REMOTE_ENV}; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${XYH_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${XYH_BASE}/bin:${PATH}"
    export PYTHONPATH="${XYH_BASE}:${XYH_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${XYH_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${XYH_BASE}/modules" ); do
            cf_init_submodule "${XYH_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${LAW_HOME:-${XYH_BASE}/.law}"
    export LAW_CONFIG_FILE="${LAW_CONFIG_FILE:-${XYH_BASE}/law.cfg}"
...
main() {
    ...
    if setup_xyh "$@"; then
        cf_color green "xyh analysis successfully setup"
        return "0"
    else
        local code="$?"
