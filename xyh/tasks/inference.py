...
from xyh.tasks.base import XYHTask
from xyh.hist_util import apply_rebinning_edges
#import xyh.inference.constants as const
...
class ModifyDatacardsFlatRebin(
    ...
    def resolve_param_values(cls, params):
        ...
        min_bkg_events_map = getattr(config_inst.x, "min_bkg_events_per_category", None)
        if min_bkg_events_map:
            config_inst.x.min_bkg_events_per_category = resolve_category_groups(
                min_bkg_events_map,
                config_inst,
            )
    ...
    def get_min_bkg_events(self, default_min_events: int = 3) -> int:
        """Return the minimum background events required per bin for the current category."""
        config_inst = self.config_insts[0]
        config_category = self.branch_data.inf_cat.config_data[config_inst.name].category
        per_category = getattr(config_inst.x, "min_bkg_events_per_category", {}) or {}
        min_events = per_category.get(config_category, default_min_events)
        try:
            min_events = int(min_events)
        except (TypeError, ValueError):
            logger.warning(
                f"Invalid min_bkg_events '{min_events}' for category {config_category}; "
                f"defaulting to {default_min_events}.",
            )
            min_events = default_min_events
        if min_events < 1:
            logger.warning(
                f"Configured min_bkg_events {min_events} for category {config_category} is < 1; "
                f"defaulting to {default_min_events}.",
            )
            min_events = default_min_events
        return min_events
    ...
    def get_rebin_processes(self):
        ...
        config_inst = self.config_insts[0]
        config_category = self.branch_data.inf_cat.config_data[self.config_insts[0].name].category
        processes = self.branch_data.inf_cat.processes.copy()
    ...
    def run(self):
        import uproot
        inputs = self.input()
        datacards_input = inputs["datacards"]
        def fetch(cat_name):
            if hasattr(datacards_input, "targets"):
                return datacards_input.targets.get(cat_name)
            return datacards_input.get(cat_name)

        branch_key = self.branch_data.inf_cat.name
        candidates = [branch_key, branch_key.replace("__", "_")]
        branch_inputs = None
        for key in candidates:
            branch_inputs = fetch(key)
            if branch_inputs:
                break

        if branch_inputs is None and isinstance(datacards_input, dict) and "card" in datacards_input:
            branch_inputs = datacards_input

        if branch_inputs is None:
            available = []
            if hasattr(datacards_input, "targets"):
                available = list(datacards_input.targets.keys())
            elif isinstance(datacards_input, dict):
                available = list(datacards_input.keys())
            raise KeyError(f"Datacard inputs missing category '{branch_key}'. Available: {available}")
        outputs = self.output()
        ...
        with uproot.open(inp_shapes.fn) as f_in:
            ...
            if _norm(cat_name) != _norm(self.branch_data.inf_cat.name):
                raise Exception(
                    f"Category name in the histograms {cat_name} does not agree with the "
                    f"datacard category name {self.branch_data.inf_cat.name}",
                )
            ...
            signal_hists = [nominal_hists[proc.name] for proc in signal_processes if proc.name in nominal_hists]
            if signal_hists:
                signal_hist = sum(signal_hists[1:], signal_hists[0]) if len(signal_hists) > 1 else signal_hists[0]
            else:
                # Fallback: use the total histogram itself when no dedicated signal histograms exist
                signal_hist = rebin_hist.copy()

            logger.info(f"Finding rebin values for category {cat_name} using processes {rebin_inf_proc_names}")
            config_inst = self.config_insts[0]
            disable_rebin_blinding = getattr(config_inst.x, "disable_rebin_blinding", False)
            if (not disable_rebin_blinding) and "data_obs" in proc_names and self.partially_unblinded:
            ...
            rebin_values = get_rebin_values(
                ...
                min_bkg_events=self.get_min_bkg_events(),
...
class PlotShiftedInferencePlots(
    ...
    Example task call:
    law run xyh.PlotShiftedInferencePlots --custom-style-config legend_single_col \
    ...
    def create_branch_map(self):
        # TODO: branch data should be split into config group + category
