...
from xyh.util import lv_xyzt, lv_mass
...
# nominal-only JEC for data
# MC should use the full JEC calibrator so that JEC uncertainty columns are produced
jec_data_nominal = jec.derive("jec_data_nominal", cls_dict={"uncertainty_sources": []})
...
def jet_energy(self: Calibrator, events: ak.Array, **kwargs) -> ak.Array:
  ...
  For MC, run full JEC including configured uncertainty sources, followed by JER.
  For data, run only nominal JEC without JEC uncertainty variations.
  """
  if self.dataset_inst.is_mc:
    events = self[jec](events, **kwargs)
    events = self[jer](events, **kwargs)
  else:
    events = self[jec_data_nominal](events, **kwargs)
...
def jet_energy_init(self: Calibrator) -> None:
  if getattr(self, "dataset_inst", None) and self.dataset_inst.is_mc:
    ...
    self.uses |= {jec_data_nominal}
    self.produces |= {jec_data_nominal}
...
@calibrator(
  uses={
    ...
    "Jet.{muonIdx1,muonIdx2,electronIdx1,electronIdx2}",
    # PF energy fractions
    "Jet.{chEmEF,muEF}",
    attach_coffea_behavior,
  },
  produces={
    "Jet.{pt,eta,phi,mass,rawFactor}",
    "Jet.{chEmEF,muEF}"
  },
)
