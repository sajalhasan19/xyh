...
Environment overrides:
  ...
  EVAL_X, EVAL_Y  Conditioning mass used for the impact evaluation
  ...
  REPEATS         Permutation repeats per feature
  MAX_EVENTS      Max combined events to evaluate
  SEED            Optional seed passed to compute_feature_impacts.py
  PLOT            If 1, also create per-fold and combined plots
...
for fold in $FOLDS; do
  ...
  json_path="${MODEL_ROOT}/mlmodel_f${fold}of${TOTAL_FOLDS}_feature_impacts.json"
  signal_jsons+=("$json_path")

  if [[ $PLOT == "1" ]]; then
    bash "${script_dir}/PlotVariableImpacts.sh" "$json_path"
  fi
done

if [[ $PLOT == "1" && ${#signal_jsons[@]} -gt 0 ]]; then
  bash "${script_dir}/PlotVariableImpactsCombined.sh" "${signal_jsons[@]}"
fi

echo "Completed feature-impact runs for folds: ${FOLDS}"
